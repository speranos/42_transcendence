...
@WebSocketGateway({
	...
	export class Classic implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleCreateRoom(client:any, arg : any)
	{
		...
		if (this.socketIds.length < 2) {
			if (this.enter)
			{
				...
				this.games.set(ids[this.j], this.game);
				this.games.get(ids[this.j]).player1 = client.id;
				this.games.get(ids[this.j]).socket1 = client;
				this.roomname1.set(client.id, ids[this.j]);
				this.gamerun.set(ids[this.j], 1);
				this.socketIIDs[0].socketid = this.games.get(ids[this.j]).player1;
				this.games.get(ids[this.j]).student1 = this.socketIIDs[0];
				while (this.socketIIDs.length > 0)
				{
					this.socketIIDs.pop();
				}
				this.enter = false;
				this.games.get(ids[this.j]).two = false;
				client.emit("am waiting");
				this.forbiden = false;
				client.join(roomName);
			}
			else if (this.socketIIDs[0] && (this.socketIIDs[0].playerid != this.games.get(ids[this.j]).student1.playerid))
			{
				this.socketIds.push(client.id);
				this.games.get(ids[this.j]).player2 = client.id;
				this.socketIIDs[0].socketid = client.id;
				this.games.get(ids[this.j]).student2 = this.socketIIDs[0];
				while (this.socketIIDs.length > 0)
				{
					this.socketIIDs.pop();
				}
				this.games.get(ids[this.j]).socket2 = client;
				this.roomname1.set(client.id,ids[this.j]);
				this.gamerun.set(ids[this.j], 1);
				this.games.get(ids[this.j]).two = true;
				this.first = 0;
			...
			if (this.socketIds.length === 2) {
				...
				await this.prisma.user.update({
					where: {
						userID: this.games.get(ids[this.j]).student2.playerid,
					},
					data: {
						userInGame : true,
					}
				...
				await this.prisma.user.update({
					where: {
						userID: this.games.get(ids[this.j]).student1.playerid,
					},
					data: {
						userInGame : true,
						userWaiting: false,
				...
				await this.prisma.games.create ({
					data: {
						gameID: ids[this.j],
						player1ID : this.games.get(ids[this.j]).student1.playerid,
						player2ID : this.games.get(ids[this.j]).student2.playerid,
					}
				});
				this.games.get(ids[this.j]).socket2.join(roomName);
				this.server.to(roomName).emit('matchFound', ids[this.j], this.games.get(ids[this.j]).student1.playeravatar,this.games.get(ids[this.j]).student2.playeravatar);
				this.j++;
				this.socketIds.pop();
				this.socketIds.pop();
			}
